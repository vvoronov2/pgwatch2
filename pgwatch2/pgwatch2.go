...
stmt_close:
	...
	if err == nil {
		if len(storeMessages) == 1 {
			log.Infof("wrote %d/%d rows to Postgres for [%s:%s] in %dus", rows_batched, total_rows,
				storeMessages[0].DBUniqueName, storeMessages[0].MetricName, t_diff.Nanoseconds()/1000)
		} else {
			log.Infof("wrote %d/%d rows from %d metric sets to Postgres in %dus", rows_batched, total_rows,
				len(storeMessages), t_diff.Nanoseconds()/1000)
		}
		lastSuccessfulDatastoreWriteTime = t1
	}
	return err
...
func main() {
	...
	if opts.Datastore == "graphite" {
		...
		//log.Fatal("OK")
		log.Info("Postgres MetricsDB connection OK")
